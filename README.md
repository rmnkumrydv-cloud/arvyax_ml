...
5. Live UI Demo
> 🚀 **App is live at: [https://arvyaxml.streamlit.app/](https://arvyaxml.streamlit.app/)**
No setup needed — open the link and use it directly in your browser.
Or run locally:
```bash
...
Noisy text — "ok", "fine", "still off" carry near-zero signal
Missing data — face_emotion_hint missing 10% train / 16% test
Conflicting signals — high stress metadata but positive text
Imperfect labels — reflection_quality = vague/conflicted means labels are unreliable
...
Priority order:
Safety signals (extreme stress / sleep deprivation)
Contradiction signals (high stress AND high energy)
...
Streamlit UI demo	`app_ui.py`	🚀 Live at arvyaxml.streamlit.app or `streamlit run app_ui.py`
---
Robustness Handling
